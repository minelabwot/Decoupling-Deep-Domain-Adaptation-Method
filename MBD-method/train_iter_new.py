...
def train_dann(model, config, DEVICE):
    ...
    def train_stage1(model, optimizer_feature, optimizer_classifier, optimizer_domain, scheduler_feature, scheduler_classifier, src_train_feature, src_train_label, tar_train_feature, tar_train_label, config, DEVICE):
        ...
        for epoch in range(config['nepoch']):
            ...
            reoversampling_epoch_list = [0] + [i * 10 for i in range(10, 60)]
            ...
            if epoch in reoversampling_epoch_list:
                ...
                with torch.no_grad():
                    ...
                    # SMOTE过采样
                    # regular-SMOTE
                    x_src_reshape_resampled, y_src_reshape_resampled = SMOTE(sampling_strategy=thisbatch_dict).fit_resample(src_full_embedding.reshape((-1, 320)), src_full_label.reshape((-1)))
                    # Borderline1-SMOTE
                    ...
                    # 记录过采样结果
                    x_oversampled_embedding = torch.from_numpy(x_src_reshape_resampled.reshape((-1, 320, 1))).to(DEVICE)
                    ...
                    X_resampled = torch.from_numpy(x_src_reshape_resampled.reshape((-1, 320, 1)))
